 ...
 * @brief Simulation of a viscoelastic drop using Basilisk C with a Weissenberg number sweep.
 *  This code simulates the dynamics of a viscoelastic drop in an axisymmetric domain.
 * The simulation parameters include various dimensionless numbers such as the Weissenberg number (Wi), Ohnesorge number (Oh), 
 * and Elastic number (El). The code also handles two-phase flow with surface tension and viscoelastic effects.
...
int MAXlevel;
double tmax, We, Ohd, Ohs, El, Wi, Bo, Ldomain;
...
int main(int argc, char const *argv[]) {
  ...
  El = 1e2;  // Elastic number
  Wi = 1e-2;  // Weissenberg number
  ...
  fprintf(ferr, "Level %d tmax %g. We %g, Ohd %3.2e, Ohs %3.2e, El %g, Wi %g, Lo %g\n", 
          MAXlevel, tmax, We, Ohd, Ohs, El, Wi, Ldomain);
...
event logWriting(i++) {
  ...
  if (pid() == 0) {
    if (i == 0) {
      ...
      fp = fopen("log", "w");
      fprintf(fp, "Level %d tmax %g. We %g, Ohd %3.2e, Ohs %3.2e, El %g, Wi %g, Lo %g\n", 
              MAXlevel, tmax, We, Ohd, Ohs, El, Wi, Ldomain);
